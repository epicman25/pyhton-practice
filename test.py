...
recipient_contact_number = ["+919902990611","+919491846035","+919491621236"]  # Recipient's phone number for notifications
tickets_date = "2024-05-04"  # Desired date for ticket notifications, format "YYYY-MM-DD"
num_of_messages_to_send = 15  # Number of notification messages to send once tickets are available
interval_between_messages = 15  # Seconds between each notification message

# Twilio account details for sending SMS
account_sid = 'AC95c8c353d40b890d3df35dd33dd5dd41'  # Twilio account SID
auth_token = '4beefccdd896e7211d813a1d2e7cc4ce'  # Twilio auth token
client = Client(account_sid, auth_token)  # Twilio client initialization
twilio_contact_number = "+13344535329"  # Twilio phone number used for sending SMS
...
fetch_status_delay = 20  # Delay in seconds for script re-execution if tickets are not available
        ...
        if formatted_date == tickets_date:
            ...
            for message_num in range(num_of_messages_to_send):
                message = client.messages.create(
                    from_=twilio_contact_number,
                    body=f'The match tickets for {tickets_date} are available. Login to {rcb_tickets_page_url} to book the tickets immediately.',
                    to=recipient_contact_number[0]
                )
                message = client.messages.create(
                    from_=twilio_contact_number,
                    body=f'The match tickets for {tickets_date} are available. Login to {rcb_tickets_page_url} to book the tickets immediately.',
                    to=recipient_contact_number[2]
                )
                # message = client.messages.create(
                #     from_=twilio_contact_number,
                #     body=f'The match tickets for {tickets_date} are available. Login to {rcb_tickets_page_url} to book the tickets immediately.',
